...
# Import data
mnist = input_data.read_data_sets('..\CommonDataset\mnist', one_hot=True) # object, not data

# construction phase
X = tf.placeholder(tf.float32, [None, 784], name = "input_1")
x_image = tf.summary.image('input', tf.reshape(X, [-1, 28, 28, 1]), max_outputs=3)  # max_outputs=3
...
writer = tf.summary.FileWriter('./tensorbaord-logs')  # create writer
...
saver.save(sess, ".\checkpoint\model_beginner") # checkpoint라는 디렉토리에 model_beginner라는 checkpoint file이 생긴다.
