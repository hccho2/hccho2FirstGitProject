...
##################################################################################
Multi Cell인 경우(별로 다른 거는 없다)
2단이면 2개의 init state가 필요하다.

h1: batch1, batch2, batch3, ....
h2: batch1, batch2, batch3, ...
    
beam width만큼 각각의 batch를 복사해 주면 된다
baem_width가 3이면

h1: batch1,batch1,batch1, batch2,batch2,batch2, batch3,batch3,batch3, ....
h2: batch1,batch1,batch1, batch2,batch2,batch2, batch3,batch3,batch3, ....
    ...
    def __init__(self,batch_size,hidden_dim,output_dim,embedding_dim,h0,seq_length=None,is_training=True):
        ...
        with tf.variable_scope('DynamicDecoder',reuse = tf.AUTO_REUSE) as scope:
            ...
            if is_training:
                ...
                decoder = tf.contrib.seq2seq.BeamSearchDecoder(cell=cell,embedding=embedding,start_tokens=tf.tile([SOS_token], [batch_size]),
                                                               end_token=EOS_token,initial_state=initial_state,beam_width=beam_width,output_layer=output_layer)
                self.outputs, self.last_state, self.last_sequence_lengths = tf.contrib.seq2seq.dynamic_decode(decoder=decoder,output_time_major=False,impute_finished=False,maximum_iterations=20)  # impute_finished=True ==> error occurs
...
class DecoderPrenetWrapper(RNNCell):
    ...
    def __init__(self, cell,is_training, prenet_sizes, dropout_prob):
...
class BeamDecode():
    def __init__(self,batch_size,hidden_dim,output_dim,embedding_dim,h0,seq_length=None,is_training=True):
        ...
        with tf.variable_scope('DynamicDecoder',reuse = tf.AUTO_REUSE) as scope:
            ...
            cell = DecoderPrenetWrapper(cell, is_training, [embedding_dim,embedding_dim],0.2)
